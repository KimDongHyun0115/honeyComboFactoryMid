...
@WebServlet("/HotIssueProductServlet")
public class HotIssueProductServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public HotIssueProductServlet() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("hotCUProductPageNumber") != null) {
			...
			pageNumber = Integer.parseInt(request.getParameter("hotCUProductPageNumber"));
			contentCount = Integer.parseInt(request.getParameter("hotCUProductContentCount"));
		} else if (request.getParameter("hotGSProductPageNumber") != null) {
			store = "GS25";
			// 클라이언트에서 보내는 페이지 번호와 데이터 수
			pageNumber = Integer.parseInt(request.getParameter("hotGSProductPageNumber"));
			contentCount = Integer.parseInt(request.getParameter("hotGSProductContentCount"));
		}
